...
void perform_computations(double* a, double* b, double* c, int32_t* d, float* e) {
    ...
    __m256i vd = _mm256_load_si256(reinterpret_cast<__m256i*>(d));
    __m256 vef = _mm256_cvtepi32_ps(vd);
    ...
    __m512d vloadu = _mm512_loadu_pd(c);

    // Store results back to memory
    _mm512_store_pd(b, vfma);
    _mm512_store_pd(c, vset);
    _mm512_store_pd(a + 8, vmul);
    _mm512_store_pd(b + 8, vsub);
    _mm512_store_pd(c + 8, vfmadd);
    _mm512_store_si512(reinterpret_cast<__m512i*>(d), vcvt64);
    _mm256_store_si256(reinterpret_cast<__m256i*>(d + 16), vcvti);
    _mm256_store_ps(e, vef);
}
